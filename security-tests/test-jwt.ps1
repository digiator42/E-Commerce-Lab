...
try {
    ...
    Write-Host "✓ Obtained valid JWT token" -ForegroundColor Green
} catch {
    Write-Host "✗ Could not obtain token. Please ensure test user exists." -ForegroundColor Red
    exit 1
}
...
try {
    $response = Invoke-RestMethod -Uri "$BaseUrl/api/auth/is-logged-in" -Method Get -Headers @{Authorization = "Bearer $noneToken"} -TimeoutSec 5
    Write-Host "  ✗ VULNERABLE: Token with 'none' algorithm was accepted!" -ForegroundColor Red
    $testResults += [PSCustomObject]@{Test = "None Algorithm"; Status = "FAIL"; Details = "Accepted token with none algorithm"}
} catch {
    Write-Host "  ✓ Secure: Token with 'none' algorithm rejected" -ForegroundColor Green
...
try {
    ...
    Write-Host "  ✗ VULNERABLE: Expired token was accepted!" -ForegroundColor Red
    $testResults += [PSCustomObject]@{Test = "Expired Token"; Status = "FAIL"; Details = "Expired token accepted"}
} catch {
    Write-Host "  ✓ Secure: Expired token rejected" -ForegroundColor Green
...
if ($parts.Length -eq 3) {
    ...
    try {
        $response = Invoke-RestMethod -Uri "$BaseUrl/api/auth/is-logged-in" -Method Get -Headers @{Authorization = "Bearer $tamperedToken"} -TimeoutSec 5
        Write-Host "  ✗ VULNERABLE: Tampered token was accepted!" -ForegroundColor Red
        $testResults += [PSCustomObject]@{Test = "Token Tampering"; Status = "FAIL"; Details = "Tampered token accepted"}
    } catch {
        Write-Host "  ✓ Secure: Tampered token rejected" -ForegroundColor Green
...
try {
    $response = Invoke-RestMethod -Uri "$BaseUrl/api/auth/is-logged-in" -Method Get -TimeoutSec 5
    Write-Host "  ✗ INSECURE: Endpoint accessible without token!" -ForegroundColor Red
    $testResults += [PSCustomObject]@{Test = "Authentication Required"; Status = "FAIL"; Details = "Endpoint accessible without token"}
} catch {
    Write-Host "  ✓ Secure: Token required for access" -ForegroundColor Green
...
try {
    $response = Invoke-RestMethod -Uri "$BaseUrl/api/auth/is-logged-in" -Method Get -Headers @{Authorization = "Bearer $wrongSignatureToken"} -TimeoutSec 5
    Write-Host "  ✗ VULNERABLE: Token with wrong signature was accepted!" -ForegroundColor Red
    $testResults += [PSCustomObject]@{Test = "Signature Validation"; Status = "FAIL"; Details = "Wrong signature accepted"}
} catch {
    Write-Host "  ✓ Secure: Wrong signature rejected" -ForegroundColor Green
...
if ($validToken) {
    try {
        $response = Invoke-RestMethod -Uri "$BaseUrl/api/admin/users" -Method Get -Headers @{Authorization = "Bearer $validToken"} -TimeoutSec 5
        Write-Host "  ✗ VULNERABLE: Regular user accessed admin endpoint!" -ForegroundColor Red
        $testResults += [PSCustomObject]@{Test = "Privilege Escalation"; Status = "FAIL"; Details = "Regular user accessed admin area"}
    } catch {
        if ($_.Exception.Response.StatusCode -eq 403) {
            Write-Host "  ✓ Secure: Admin access properly restricted" -ForegroundColor Green
            $testResults += [PSCustomObject]@{Test = "Privilege Escalation"; Status = "PASS"; Details = "Role-based access enforced"}
        } else {
            Write-Host "  ⚠ Admin access check: HTTP $($_.Exception.Response.StatusCode)" -ForegroundColor Yellow
            $testResults += [PSCustomObject]@{Test = "Privilege Escalation"; Status = "WARN"; Details = "Unexpected response"}
        }
    }
}
...
if ($validToken) {
    ...
    try {
        ...
        try {
            $response = Invoke-RestMethod -Uri "$BaseUrl/api/auth/is-logged-in" -Method Get -Headers @{Authorization = "Bearer $validToken"} -TimeoutSec 5
            Write-Host "  ✗ VULNERABLE: Blacklisted token was accepted!" -ForegroundColor Red
            $testResults += [PSCustomObject]@{Test = "Token Blacklisting"; Status = "FAIL"; Details = "Token still valid after logout"}
        } catch {
            Write-Host "  ✓ Secure: Blacklisted token rejected" -ForegroundColor Green
            $testResults += [PSCustomObject]@{Test = "Token Blacklisting"; Status = "PASS"; Details = "Token properly invalidated"}
        }
    } catch {
        Write-Host "  ⚠ Could not test blacklisting $_.Exception.Message" -ForegroundColor Yellow
...
if ($failed -gt 0) {
    ...
    Write-Host "`n✓ JWT implementation appears secure" -ForegroundColor Green
}